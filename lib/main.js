...
require("./chrome-protocol").register("foo", require("sdk/self").data.url());
...
function lookForMagicObject(iframeWindow, cx, analyzer) {
  ...
  for (var i in analyzer.graph) {
    ...
    if ((o.name == "JS Object (Object)" || o.name == "JS Object (Proxy)") &&
        jsapi.JS_IsDeadWrapper(jsapi.getPointerForAddress(o.address))) {
      console.log("DEAD wrapper!!!");
    }
...
function openToolbox() {
  ...
  gDevToolsBrowser.selectToolCommand(gBrowser, "mem-panel" + require("self").id);
}
